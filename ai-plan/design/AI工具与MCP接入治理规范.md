...
- `headroom`
  - 等级：`L1` 默认；启用 `--memory`、`--learn`、自动写 `AGENTS.md` / Codex `instructions.md` 或其它写入型上下文能力时视为 `L2`
  - 用途：本地 Codex wrapper、按需 MCP 压缩、检索和统计，降低 AI 辅助开发时的大型 tool output / log / file context token 压力；MCP 工具默认包括 `headroom_compress`、`headroom_retrieve` 和 `headroom_stats`。
  - 安装方式：用户本地 Python 工具安装；当前已验证入口：
  ...
  - 说明：上游文档声明 MCP 轻量安装为 `headroom-ai[mcp]`，但当前 `0.24.0` CLI 启动会导入 proxy 模块；本仓库按已验证的 `[proxy]` extra 记录，避免 MCP 指向不可启动的 CLI。
  - CLI wrapper 推荐入口：
  ...
  - 采用条件：任务需要处理大体量工具输出、日志、检索结果或跨 agent context，且普通 `rg`、真实文件读取和现有 MCP 不能有效控制上下文成本。
  - 约束：
    - 默认关闭匿名 telemetry：使用 `HEADROOM_TELEMETRY=off` 或等价本地配置。
    - 默认只允许压缩、检索和统计；不得默认启用 `headroom wrap codex --memory`、`headroom wrap codex --learn` 或 `headroom learn --apply`。
    - Headroom memory 不得替代 `ai-plan/public/**` 的 topic recovery 真值。
    - `headroom learn` 只能作为候选 lesson 生成器；任何写入 `AGENTS.md`、`ai-plan/lessons/**`、设计文档或 Codex `instructions.md` 的内容都必须先人工确认具体 diff，再走 `graft-lessons-learned` 或对应治理路径。
