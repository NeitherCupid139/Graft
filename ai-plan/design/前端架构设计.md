...
当前仓库向该结构收敛时，允许经历一个“最小模块边界过渡态”：

* 先落地 `web/src/modules/<name>/` 作为模块注册与边界声明入口
* 旧页面、旧 API 或旧契约可以暂时继续保留在现有 `pages/`、`api/`、`contracts/` 下
* 但新的 bootstrap 动态路由接入、模块 owned scope 和后续多 worktree 边界，必须优先从 `modules/` 入口收口
* 在过渡态结束前，不允许继续把新的动态路由白名单直接堆回共享壳层文件

多 worktree 准备阶段的 `web` 目录所有权补充约束：

* `layouts/`、`router/`、`locales/`、全局 `store`、平台级 `contracts/` 属于 `shell-owned` 目录
* `web/src/modules/<name>/` 属于模块自拥有边界，模块内部应承载该功能的页面、API、局部类型、模块私有稳定契约与模块消息源
* 后续 long-lived worktree 的默认 `web` owned scope 按模块粒度冻结，不按 `api/`、`pages/`、`store/` 等技术层切分
...
* 模块应通过显式注册层向壳层声明可接入的 bootstrap 动态路由
